...
### Step 5.4 — Visual Contrast Check (MANDATORY)
...
- Dark text on dark backgrounds → the build uses `textHeading`/`textBody` (dark colors) on sections with dark images or dark bg. Fix by:
...
- Light text on light backgrounds → same idea, use dark text variants
